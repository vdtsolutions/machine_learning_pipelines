# ================= RULE FUNCTIONS ================= #
# WIDTH VERSION (length → width only, logic unchanged)
...
# RULE 2 — MANY TESTS → ONE REF (WIDTH ERROR BASED)
def rule_two_single_block(out_df, info, ref_flag):
    ...
    for tid in info["test_ids"]:
        pred = float(info["pred_width"][tid])
        actual = float(info["actual_width"][tid])
...
# RULE 3 — ONE TEST → MANY REFS (BEST WIDTH MATCH)
...
# RULE 4 — MANY TESTS ↔ MANY REFS (DISTANCE + WIDTH HEURISTIC)
def rule_four_single_block(out_df, info, ref_flag):
    ...
    if len(unique_distances) == 1:
        ...
        pred = float(info["pred_width"][winner_tid])
        actuals = info["actual_width"][winner_tid]
