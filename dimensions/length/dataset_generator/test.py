...
# PIPE_INCH  = 12
#
# # Column names in results_matching
# LABEL_COL      = "correct"
...
#     """Extract signal features from messy submatrix CSV without applymap (no warnings)"""
#
#     try:
#         df = pd.read_csv(path, header=None, dtype=str, low_memory=False)
#     except Exception as e:
#         print(f"⚠️ Failed reading submatrix {path}: {e}")
#         return None
#
#     # Convert to numpy string array
#     arr = df.to_numpy(dtype=str)
#
#     # Vectorized regex to extract numeric part
#     def extract_num(x):
#         m = re.search(r"-?\d+\.?\d*", x)
#         return float(m.group()) if m else np.nan
#
#     # Vectorized conversion
#     vec_extract = np.vectorize(extract_num)
#     mat = vec_extract(arr)
#
#     # Replace NaNs
...
#
#     # Axial footprint
#     axial_active = np.any(mat > np.percentile(mat, 50), axis=0)
...
#         print("❌ Missing:", match_path)
#         continue
#     if not os.path.exists(results_path):
#         print("❌ Missing:", results_path)
#         continue
#     if not os.path.exists(submat_dir):
#         print("❌ Missing:", submat_dir)
#         continue
#
#     # Load CSVs
...
#
#     print(f"PTT {ptt} label distribution:")
...
#
#         # Robust submatrix match (no glob parentheses bug)
...
#
#             # Submatrix features
#             "axial_span_sub": f["axial_span_sub"],
#             "mean_amp": f["mean_amp"],
#             "max_amp": f["max_amp"],
#             "std_amp": f["std_amp"],
#             "energy": f["energy"],
...
#
# if "correct" in master_df.columns:
#     print("\nFinal Label Distribution:")
#     print(master_df["correct"].value_counts())
# else:
#     print("⚠️ No 'correct' column found in output (dataset empty?)")
...
# Optional imports (script won't crash if missing)
try:
...
def fft_features(signal):
    if not SCIPY_OK or len(signal) < 3:
        return {"fft_peak_freq": np.nan, "fft_peak_mag": np.nan, "fft_energy": np.nan}
...
def cwt_features(signal):
    if not PYWT_OK or len(signal) < 3:
        return {"cwt_energy": np.nan, "cwt_peak_scale": np.nan}
...
def extract_submatrix_features(path):
    ...
    # Extract numeric from messy strings like 129a_extra
    def extract_num(x):
        m = re.search(r"-?\d+\.?\d*", x)
        return float(m.group()) if m else np.nan
